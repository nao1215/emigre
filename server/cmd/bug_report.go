...
`
	...
	buf.WriteString(fmt.Sprintf("## emigre version\n%s\n\n", cmd.Version))
	...
	body := buf.String()
	url := "https://github.com/nao1215/emigre/server/issues/new?title=[Bug Report] Title&body=" + url.QueryEscape(body)

	if !openBrowser(url) {
		fmt.Print("Please file a new issue at https://github.com/nao1215/emigre/server/issues/new using this template:\n\n")
		fmt.Print(body)
	}
